...
public class BusManager {
    ...
    public void pesanTiket() {
        ...
        System.out.println("Kapasitas      : " + bus.getKursiTersedia() + " kursi tersedia dari " + bus.getKursiTersedia());

        System.out.print("\nNomor kursi (1-" + bus.getKursiTersedia() + "): ");
        int kursi = scanner.nextInt();

        if (!bus.isKursiAvailable(kursi)) {
            System.out.println("Kursi tidak tersedia atau nomor tidak valid!");
            return;
        }
        ...
        double hargaMakan = paketMakan ? 35000 : 0;
    ...
    public void showNota(Transaksi t) {
        ...
        System.out.printf("Paket Makan    : Rp%,.0f%n", t.isPaketMakan() ? 35000 : 0);
        System.out.printf("Subtotal       : Rp%,.0f%n", t.getBusDipilih().getHarga() + (t.isPaketMakan() ? 35000 : 0));
    ...
    public void showDaftarTrx() {
        ...
        for (Transaksi t : daftarTransaksi) {
            System.out.printf("%-10s %-15s %-12s %-20s %-8d Rp%,-10.0f%n",
                    t.getIdTransaksi(),
                    t.getNamaPelanggan().length() > 13 ? t.getNamaPelanggan().substring(0,13)+".." : t.getNamaPelanggan(),
                    t.getBusDipilih().getNamaBus().length() > 10 ? t.getBusDipilih().getNamaBus().substring(0,10)+".." : t.getBusDipilih().getNamaBus(),
